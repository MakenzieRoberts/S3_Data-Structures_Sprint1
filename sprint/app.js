...
const getMessage = (request, response) => {
	...
	if (structure != 1 || structure != 2) {
		response
			.status(200)
			.send(
				`Invalid Data Structure. <br> Please Enter 1 to receive a message from the stack, or 2 to receive a message from the queue.`
			);
	} else {
		pool.query(
			...
			(error, results) => {
				...
				if (structure === 1) {
					...
					if (queueResult === undefined) {
						...
						response.status(200).send(`DATA: ${parsedResult.data}`);
						console.log(
							"Message data displayed in browser: ",
							parsedResult.data
						);
...
const addMessage = (request, response) => {
	...
	if (structure != 1 || structure != 2) {
		response
			.status(200)
			.send(
				`Invalid Data Structure. <br> Please Enter 1 to add your message to the stack, or 2 to add your message to the queue.`
			);
	} else {
		pool.query(
